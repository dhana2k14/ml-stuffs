...
date_calender<-function(){
  ...
  myconn <-odbcConnect("RETAIL", uid="MRO_INV_OPT", pwd="MRO_INV")
  
  
  
  query="SELECT min([INVOICE_DATE]) as min_date,max([INVOICE_DATE]) as max_date
  from EGL_ORDERS"
...
#   
#   
#   query="SELECT distinct([Store_No]) as store_name
...
#  from [dbo].[Main_data_retail_analytics_VERSION_2]"
  ...
  query="SELECT 'All Items' as item_name from dual"  
  ...
  result_item <- "EGL"  ## Store name is hard-coded
...
#  
 ...
 SQLquery <- sprintf("SELECT 
                     count(1) as frequency_data,
                     sum(document_selling_price) as monetary_data,
                     max(invoice_date) as recency_date,
                     Admin_Account_number as Loyalty_Card_No,
                     'Studio' as Store_No,
                     'All Items' as Item_Category_Code_Desc 
                     from EGL_ORDERS
                     where invoice_date >= %s 
                     And invoice_date < %s 
                     # and Item_Category_Code_Desc IN (%s)
                     # And Store_No IN (%s) 
                     group by Admin_Account_number",vcharStartDate1,vcharEndDate1)

  SQLconn <-odbcConnect("RETAIL", uid="MRO_INV_OPT", pwd="MRO_INV")
  ...
  vmatRecency= cbind("Recency"=vdifftimeRecent,"Loyalty_Card_No"=as.character(dfSQLResult$Loyalty_Card_No))
  vmatFrequency <- cbind("Frequency"=dfSQLResult$frequency_data,"Loyalty_Card_No"=as.character(dfSQLResult$Loyalty_Card_No))
  vmatMonetary<-cbind("Monetary"=dfSQLResult$monetary_data,"Loyalty_Card_No"=as.character(dfSQLResult$Loyalty_Card_No))
