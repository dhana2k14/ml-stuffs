---
title: "Getting Started with Spatial Data Analysis with R"
...
## Agenda
1. Why R for spatial analysis, 
2. A typical data analysis life-cycle: 
...
## History
Before 2003:  
* there were numerous packages for spatial data; These packages made different assumptions about how spatial data were organised
* R itself had no capabilities for distinguishing coordinates from other numbers
* Methods for plotting spatial data and other tasks were scattered, made different assumptions on the organisation of the data, and were rudimentary.

In 2003:  
* A group of R developers came together & wrote the R package sp to extend R with classes and methods for spatial data.
* Classes specify a structure and define how spatial data are organised and stored. 
* Methods are instances of functions specialised for a particular data class.

## Advantages of Using a Single Set of Classes
1. It is much easier to move data across spatial statistics packages.
2. Well-tested set of methods and functions for plotting, printing, subsetting, and summarising spatial objects, or combining (overlaying) spatial objects
3. New classes support packages with interfaces to GIS, for reading and writing GIS file formats, and for coordinate (re)projection.
4. New methods include plot methods that combine points, lines, polygons, and grids with map elements
(reference grids, scale bars, north arrows), degree symbols (as in 52 ◦ N) in axis labels, etc.

## R Packages for Spatial Data Analysis
Some of my favorite packages for spatial data analysis include:

* [sp](https://cran.r-project.org/web/packages/sp/index.html): Provides classes and methods for spatial data; utility functions for plotting maps etc.
* [rgdal](https://cran.r-project.org/web/packages/rgdal/index.html): For importing and exporting different geospatial data formats; Coordinate Reference Systems; etc.
* [rgeos](https://cran.r-project.org/web/packages/rgeos/index.html): For handling operations on topologies.
* [ggplot2](http://ggplot2.org/): For data visualisation by [Hadely Wickham](http://had.co.nz/)
* [ggmap](https://cran.r-project.org/web/packages/ggmap/index.html): To visualize spatial data ontop of static maps from sources like Google Maps, Open Steet Maps, cloudmade and stamen.
* [leaflet](http://rstudio.github.io/leaflet/): Provides functions to control and integrate Leaflet, a JavaScript library for interactive maps, within R.
* [lubridate](https://cran.r-project.org/web/packages/lubridate/index.html): For manipulating dates and times.


# Data Preparation

## The Data (1/3)
We shall use crime data from the [Houston Police Department](http://www.houstontx.gov/police/cs/stats2.htm) collected over the period of January 2010 - August 2010.
...
## A quick glance at our data (1/3)
...
## A quick glance at our data (2/3)
```{r}
summary(crime_df)
```

## A quick glance at our data (3/3)
...
## let's convert our local dataframe.
...
## Spatial Points Dataframe (2/7)
* Notice that we used some new functions namely SpatialPoints, SpatialPointsDataFrame, CRS and proj4string.  
* These come from the "sp" package for spatial analysis.   
* The foundation class from this package is "Spatial" from which other sub classes are generated including the following:
```{r sp_classes}
getClass("Spatial")
```

## Spatial Points Dataframe (3/7)
1. We can see that **SpatialPoints** and **SpatialPointsDataFrame** are subclass of the **Spatial** class.
2. This kind of class system provides richer classes & methods for working with spatial data types.
3. The sp package provides classes and methods for points, lines, polygons, and grids
4. Adopting a single set of classes for spatial data offers a number of important advantages

## Spatial Points Dataframe (4/7) 
* Advantages of using a single set of classes:
1. It is much easier to move data across spatial statistics packages.
2. Well-tested set of methods and functions for plotting, printing, subsetting, and summarising spatial objects, or combining (overlaying) spatial objects
3. New classes support packages with interfaces to GIS, for reading and writing GIS file formats, and for coordinate (re)projection.
...
## Cordinate Reference Systems (1/3)
...
## Cordinate Reference Systems (2/3)
...
## Cordinate Reference Systems (3/3)
...
```{r write_data1}
## So we can quickly read in our processed data 
## without having to re-process it.
...
## ESRI Shapefile Polygon (2/4)
```{r}
class(texas_shp)
```

## ESRI Shapefile Polygon (3/4)
...
## Visualizing the Data Using Traditional Plot System (1/3)
...
## Visualizing the Data Using Traditional Plot System (2/3)
...
## Visualizing the Data Using Traditional Plot System (3/3)
...
## Use "get_map" command to download the images
...
m <- leaflet() %>% 
  setView(lng = -95.3698028, lat = 29.7604267, zoom = 12) %>% 
  addTiles()
m
```
