...
{
  check=getRFM(StartDate="2010-01-01"
  , EndDate="2015-02-23"
  , stores=c("S0001")
  , items=c("Activewear")
  , WeightRecency=1
  ...
  output$startDateUI <- renderUI({dateInput("startDateServ"
  
	, label="Start Date"
	, min=min(date_calender())
	, max = max(date_calender())
	, format = "yyyy-mm-dd"
	, value = min(date_calender()) )
    
  })
  
  output$endDateUI<-renderUI({
    dateInput("endDateServ"
	, label="End Date"
	, min=min(date_calender())
	, max = max(date_calender())
	, format = "yyyy-mm-dd"
	, value = max(date_calender()) )
        
  })
  
  output$storeDatasetUI <-renderUI({
    selectizeInput("storeServ"
	  ,	label="Store "
	  , choices = store_ddl() 
  ...
  output$itemcategoryUI <-renderUI({
    ...
    selectizeInput("itemCategoryServ"
	  , label="Item Category "
	  , choices = itemCategory_ddl()
	  , multiple = TRUE
	  , selected = itemCategory_ddl()[1]
    )
  ...
  output$recency<-renderUI({
   ...
   if(v$condvar1==1){
      ...
      sliderInput("recencyRange"
	  , "Recency in days"
	  , min = min(as.numeric(check$Recency))
	  , max =max(as.numeric(check$Recency))
	  , value = c(min(as.numeric(check$Recency))
	  , max(as.numeric(check$Recency))) )
   ...
   else
	{
		sliderInput("recencyRange"
		, "Recency in days"
		, min = min(as.numeric(rfmData()$Recency))
		, max =max(as.numeric(rfmData()$Recency))
		, value =c(min(as.numeric(rfmData()$Recency))
		, max(as.numeric(rfmData()$Recency))) )
	}
  })
  ...
  output$frequency<-renderUI({
    ...
    if(v$condvar1==1)
	{
      sliderInput("frequencyRange"
	  , "Frequency"
	  , min = min(as.numeric(check$Frequency))
	  , max = max(as.numeric(check$Frequency))
	  , value = c(min(as.numeric(check$Frequency))
	  , max(as.numeric(check$Frequency))))
    ...
    sliderInput("frequencyRange"
	, "Frequency"
	, min = min(as.numeric(rfmData()$Frequency))
	, max = max(as.numeric(rfmData()$Frequency))
	, value = c(min(as.numeric(rfmData()$Frequency))
	, max(as.numeric(rfmData()$Frequency))))
    }  
 ...
 output$countCustomers <- renderText({
  ...
  output$averagePurchase <- renderText({
    if(v$condvar1==1)
	{
      ...
      if(length(d[,5])!=0)
	  {
        paste(currency, round(sum(as.numeric(paste(d$Monetary)))/length(d$Monetary),2)) 
      } 
	  else 
	  {
        paste(" ")
...
output$RFM_table<- DT::renderDataTable({reactiveDataTableRFM()}
...
output$downloadDataAsCsvRFM <- downloadHandler(filename = function() 
	
	{ 
		paste(input$startDateServ,input$endDateServ,input$itemCategoryServ,input$storeServ,'.csv', sep='-') 
	}
