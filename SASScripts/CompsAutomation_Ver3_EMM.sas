...
run;
	...
	%end; 
		...
		%do n=2 %to &Obs_1;
        proc append base=C1R&n_&&Nbhd&i data=C1R1 force;
		run;
    	proc append base=C1R1_&&Nbhd&i data=C1R&n_&&Nbhd&i force;
		run;
		proc append base=C1Rw1_&&Nbhd&i data=C1Rw&n force;
		run;
		%end;
			...
			proc datasets;
			copy in=work out=&&libname move;
			select C1Rw1_&&Nbhd&i;
			quit;

			data &&libname..C1Rw1_&&Nbhd&i (Keep = InvPin SalePin);
			set &&libname..C1Rw1_&&Nbhd&i;
			run;
