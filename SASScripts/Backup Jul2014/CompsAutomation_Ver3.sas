...
run;
	...
	AdjEmv as J_AdjEmv,Comp as L_CompRating from C1Rw&n;quit;run;

	proc transpose data=SC1R&n out=TrSC1R&n (rename = (_name_=Variables Col1=COMP1 Col2 = COMP2 Col3 = COMP3 Col4=COMP4 Col5=COMP5)drop=_label_);
	var _all_;
	run;
	...
	%end; 
		...
		%do n=2 %to &Obs_1;
        proc append base=C1R1&&Suburb&i data=C1R1 force;
		run;
    	proc append base=C1R1&&Suburb&i data=C1R&n&&Suburb&i force;
		run;
		proc append base=C1Rw1&&Suburb&i data=C1Rw&n force;
		run;
		%end;

			proc datasets;
			copy in=work out=&&libname move;
			select C1R1&&Suburb&i;
			quit;

			proc datasets;
			copy in=work out=&&libname move;
			select C1Rw1&&Suburb&i;
			quit;
