...
run;
	...
	%end; 
		...
		run;
		proc append base=GC1R1&&Suburb&i data=GC1R&n&&Suburb&i force;
		run;
		proc append base=DC1R1&&Suburb&i data=DC1R&n&&Suburb&i force;
		run;
		%end;
			...
			proc datasets;
			copy in=work out=coj move;
			select GC1R1&&Suburb&i;
			quit;

			proc datasets;
			copy in=work out=coj move;
			select DC1R1&&Suburb&i;
			quit;
